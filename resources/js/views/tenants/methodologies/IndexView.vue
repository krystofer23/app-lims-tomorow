<template>
    <div class="w-full">
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden">
            <div class="p-5 border-b border-slate-200 bg-slate-100/50">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-slate-900 leading-tight">Metodologías</h2>
                        </div>
                    </div>
            ...
            <div class="p-5">
                <el-table stripe :data="methodologies" v-loading="loading" class="w-full" :header-cell-style="headerStyle"
                ...
                <div class="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <p class="text-sm text-slate-500">
                        Mostrando <span class="font-semibold text-slate-700">{{ methodologies.length }}</span> de
                    ...
                    <div class="min-w-0">
                        <div class="flex items-center gap-3">
                            <div
                                ...
                                <i class="fa-solid fa-note-sticky text-emerald-500"></i>
                            </div>

                            <div class="min-w-0">
                                <h3 class="text-base font-semibold text-slate-900 leading-5">
                                    {{ formData.id ? 'Actualizar metodología ' : 'Registrar metodología ' }}
                                </h3>
            ...
            <div class="grid grid-cols-1 gap-4">
                <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="flex items-center justify-between gap-2">
                        <label class="text-sm font-semibold text-slate-900">Metodología</label>
                    ...
                    <p class="mt-2 text-xs text-slate-500 flex items-start gap-2">
                        ...
                        Describe la metodología de forma clara y accionable.
                    </p>
                </div>
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-100/40 p-4">
                    ...
                    <p class="mt-2 text-xs text-slate-500 flex items-start gap-2">
                        ...
                        Úsalo para notas, referencias o detalles que ayuden a entender la metodología.
                    </p>
                </div>
            </div>
        </div>
...
const getMethodologies = async (page = 1) => {
    loading.value = true

    try {
        const { data } = await tenant.get(`methodologies?page=${page}`)
...
const onSubmit = async () => {
    ...
    try {
        if (formData.value.id) {
            const { data } = await tenant.put(`methodologies/${formData.value.id}`, formData.value)
            ElNotification.success(data.message)
        }
        else {
            const { data } = await tenant.post(`methodologies`, formData.value)
            ElNotification.success(data.message)
        }
...
const handleDestroy = async (id) => {
    try {
        const { data } = await tenant.delete(`methodologies/${id}`)
