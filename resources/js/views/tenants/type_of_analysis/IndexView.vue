<template>
    ...
    <div class="bg-white p-5 space-y-4">
        <div class="overflow-x-auto">
            <el-table class="border rounded-xl" v-loading="loading" stripe :data="typesOfAnalysis"
        ...
        <div class="px-2 mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p class="text-sm text-slate-500">
                Mostrando <span class="font-semibold text-slate-700">{{ typesOfAnalysis.length }}</span> de
    ...
    <el-dialog v-model="visible" width="520px" align-center :show-close="false" class="custom-dialog !rounded-lg">
        ...
        <form class="space-y-5 pt-2" @submit.prevent="onSubmit">
            ...
            <div class="rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-700">
                Este tipo de analisis podrá ser usado para generar conexiones o agrupaciones dentro del sistema.
            </div>
...
const getTypeOfAnalysis = async (page = 1) => {
    loading.value = true

    try {
        const { data } = await tenant.get(`type-of-analysis?page=${page}`, {
            params: {
        ...
        if (data.data) {
            typesOfAnalysis.value = data.data.data
            pagination.value = {
...
const onSubmit = async () => {
    ...
    try {
        if (form.value.id) {
            const { data } = await tenant.put(`type-of-analysis/${form.value.id}`, form.value)
            ElMessage.success(data.message)
        }
        else {
            const { data } = await tenant.post(`type-of-analysis`, form.value)
            ElMessage.success(data.message)
        }
        ...
        getTypeOfAnalysis(pagination.value.current_page)
    }
    catch (e) {
        handleErrorsExeption(e)
    }
...
async function handleDelete(row) {
    const ok = await confirmRef.value?.open({
        title: 'Eliminar tipo de analisis',
        message: '¿Seguro que deseas eliminar el tipo de analisis?',
    ...
    if (ok) {
        ...
        try {
            const { data } = await tenant.delete(`type-of-analysis/${row.id}`)
