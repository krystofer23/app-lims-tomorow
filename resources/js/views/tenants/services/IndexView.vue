<template>
    <div class="w-full">
        <div class="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200 overflow-hidden">
            <div class="p-5 border-b border-slate-200 bg-slate-100/50">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h2 class="text-lg font-semibold text-slate-900 leading-tight">Servicios</h2>
                        </div>
                    </div>
            ...
            <div class="p-5">
                <el-table :data="services" v-loading="loading" class="w-full" :header-cell-style="headerStyle"
    ...
    <el-dialog v-model="state" @close="handleClose" :close-on-click-modal="false"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h3 class="text-base font-semibold text-slate-900 leading-5">
                                    {{ formData.id ? 'Actualizar servicio' : 'Registrar servicio' }}
                                </h3>
        ...
        <div class="px-6 pb-6 pt-5">
            <div class="mb-5 flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <h3 class="text-base font-semibold text-slate-900">Detalle del servicio</h3>
...
const getServices = async (page = 1) => {
    ...
    try {
        const { data } = await tenant.get(`service?page=${page}`, {
            params: {
                search: filters.q
            }
        })
...
const onSubmit = async () => {
    ...
    try {
        if (formData.value.id) {
            const { data } = await tenant.put(`service/${formData.value.id}`, formData.value)
            ElNotification.success(data.message)
        }
        else {
            const { data } = await tenant.post(`service`, formData.value)
            ElNotification.success(data.message)
        }
