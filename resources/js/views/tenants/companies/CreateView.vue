...
const onSubmit = async () => {
    ...
    try {
        const { data } = await tenant.post(`company`, payload.value)
...
const loadingConsulting = ref(false)

const consultingRuc = async () => {
    loadingConsulting.value = true

    try {
        const { data } = await tenant.get(`consulting-ruc/${company.ruc}`)

        if (data.data) {
            company.business_name = data.data.razonSocial
            company.direction = data.data.direccion
        }
    }
    catch (e) {
