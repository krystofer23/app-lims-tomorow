<template>
    <custom-header title="Unidades de medida" description="Registro y control de unidades de medida."
        icon="fa-solid fa-ruler-combined">
        <div class="flex w-full flex-col gap-3 sm:flex-row sm:items-center lg:w-auto">
            <el-input v-model="filters.q" placeholder="Buscar razón social, cadena o informe..." clearable
            ...
            <el-button type="primary" @click="state = true"
    ...
    <div class="bg-white p-5 space-y-4">
        <div class="overflow-x-auto">
            <el-table class="border rounded-xl" v-loading="loading" stripe :data="unitsmeasurement"
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <h3 class="text-base font-semibold text-slate-900 leading-5">
                                    {{ formData.id ? 'Actualizar unidad' : 'Registrar unidad' }}
                                </h3>
...
const getUnitsMeasurement = async (page = 1) => {
    loading.value = true

    try {
        const { data } = await tenant.get(`units-measurement?page=${page}`, {
            params: {
                search: filters.q
            }
        })
...
const onSubmit = async () => {
    ...
    try {
        if (formData.value.id) {
            const { data } = await tenant.put(`units-measurement/${formData.value.id}`, formData.value)
            ElNotification.success(data.message)
        }
        else {
            const { data } = await tenant.post(`units-measurement`, formData.value)
            ElNotification.success(data.message)
        }
...
const handleDestroy = async (id) => {
    try {
        const { data } = await tenant.delete(`units-measurement/${id}`)
